<?php
	...
	$one=mysql_query("select * from rw_reservation where number='$number'");
	$rows=mysql_fetch_assoc($one);
	$offset=($rows['reservationbeginweek']-1)*7+$rows['reservationweekday']-1; 
	...
	if($offset>0)
	{
		$rs=mysql_query("update rw_reservation set reservationstate='已取消' where number='$number'");
		if($rs) 
