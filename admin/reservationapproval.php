...
<head>
<meta charset="utf-8">
<title>用户长期预约界面</title>
</head>
<script type = "text/javascript" language="javascript">
function jump(number)
{
	if(confirm('确定要通过该条长期预约吗？'))
		location.href='reservationadopt.php?number='+number;
}
function jump2(number)
{
	if(confirm('确定要不通过该条长期预约吗？'))
		location.href='reservationveto.php?number='+number;
}
</script>
<body bgcolor="#B9B9FF">
...
<table width="80%" border="1" align="center">
  ...
  <?php 
		...
		$rs=mysql_query("select * from rw_reservation where reservationstate='未审核' order by number");
 		while($rows=mysql_fetch_assoc($rs)){
			echo '<tr>';
			echo '<td>'.$rows['userid'].'</td>';	
			echo '<td>'.$rows['location'].'</td>';
			echo '<td>'.$rows['roomid'].'</td>';
			echo '<td>'.$rows['reservationbeginweek'].'——'.$rows['reservationendweek'].' 周'.$rows['reservationweekday'].'</td>';
			echo '<td>'.$rows['reservationinterval'].'</td>';
			echo '<td>'.$rows['reservationreason'].'</td>';
			echo '<td>'.$rows['reservationstate'].'</td>';
